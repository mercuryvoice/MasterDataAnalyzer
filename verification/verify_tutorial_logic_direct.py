...
def test_tutorial_updates(page):
    ...
    try:
        ...
        has_tutorialSteps = page.evaluate("'tutorialSteps' in window")
        print(f"Has tutorialSteps in window? {has_tutorialSteps}")
    except Exception as e:
        print("Error checking window:", e)

    print("Executing JS to simulate state...")

    # 1. Advance tutorial to load second block of steps (Task 2, 3, 4)
    # We must set currentStep to the first checkpoint before calling continueTutorial()
    page.evaluate("window.currentStep = window.tutorialSteps.findIndex(s => s.title === window.T.businessCheckpointTitle)")
    page.evaluate("continueTutorial()")
    page.wait_for_timeout(500)

    # 2. Advance to Task 4 Checkpoint
    page.evaluate("window.currentStep = window.tutorialSteps.findIndex(s => s.title === window.T.businessCheckpoint2Title)")
    page.evaluate("renderStep(window.currentStep)")
    page.wait_for_timeout(500)

    print("At Checkpoint 2. Calling endTutorial()...")
    page.evaluate("endTutorial()")
    page.wait_for_timeout(500)
    ...
    page.screenshot(path="verification/verification_en_button_direct.png")

    # 4. Verify Click Action (Return to Step 13)
    page.get_by_role("button", name=expected_button_text).click()
    page.wait_for_timeout(500)

    is_at_step_13 = page.evaluate("window.tutorialSteps[window.currentStep].title === window.T.businessFinalStepTitle")
    if is_at_step_13:
