...
MasterData.TRANSLATIONS = {
    en: {
        ...
        mfgTask2Step2Instruction: "In the <code>Data Validation settings</code> window, please configure the following:<br>1. **Source Data Select**: Click the \"Select\" button and choose the current spreadsheet file from the file picker.<br>2. **Source Data Sheet Name**: Select <code>{SOURCE_SHEET_NAME}</code><br>3. Please fill in the data range settings in the following order:<br>   - Data Import Start Row after Validation: <code>4</code><br>   - Target Data Header Start Row: <code>3</code><br>   - Source Data Header Start Row: <code>1</code><br><br>Proceed to the next step to begin setting up the 'Field Validation Conditions' and 'Validation Result Outputs'.",
        mfgTask2Step3Title: "Task 2: Field Validation Settings (3/4)",
        mfgTask2Step3Instruction: "In the Field and Validation Conditions section, the first step is to map the 'Target Column' and 'Source Column'.<br>We recommend using the <code>Auto-map Validation Fields</code> feature first, which will <b>automatically match and recommend</b> suitable headers for you.<br>For this task, the validation conditions are as follows:<br>Target Column - Source Column<br><code>B</code> - <code>B</code><br><code>C</code> - <code>C</code><br><code>D</code> - <code>D</code><br><code>E</code> - <code>E</code><br><code>G</code> - <code>A</code>",
        mfgTask2Step4Title: "Task 2: Validation Output Settings (4/4)",
        mfgTask2Step4Instruction: "Next, configure the data columns to be returned from the source upon successful validation.<br>Similarly, you can use the <code>Auto-map Output Fields</code> feature to speed up the setup.<br>The output field conditions are as follows:<br>Target Column - Source Column<br><code>H</code> - <code>F</code><br><code>I</code> - <code>G</code><br><code>J</code> - <code>H</code><br><br>Finally, we will set the \"Mismatch Info Output Column\" to <code>K</code>, so the script can write error messages there.",
        ...
        mfgTask3Step3Instruction: "Apply these settings to link the tracking number and retrieve the status:<br>1. **Source Data Select**: Click the Select button and choose the current spreadsheet file from the file picker.<br>2. **Source Data Sheet Name**: <code>{SOURCE_SHEET_NAME}</code><br>3. **Target Data Start Row**: <code>4</code><br>4. **Source Data Compare Range**: <code>A2:B22</code> (covers all data)<br>5. **Target Lookup Column**: <code>H</code> (Shipment Tracking Number)<br>6. **Source Compare Column**: <code>A</code> (Shipment Tracking Number)<br>7. **Source Return Column**: <code>B</code> (Shipment Status)<br>8. **Target Write Column**: <code>L</code> (Shipment Status)",
    ...
    'zh_TW': {
        ...
        mfgTask2Step2Instruction: "在<code>資料驗證設定</code>視窗中，請進行以下設定：<br>1. **來源資料選擇**: 點擊「選取」按鈕，這會彈出一個檔案選擇視窗，請在視窗中選擇您當前的 Google 試算表檔案。<br>2. **來源資料分頁名稱**: 選擇 <code>{SOURCE_SHEET_NAME}</code><br>3. 請依照下列設定順序填妥資料範圍設定：<br>   - 資料驗證後匯入的起始列數請填入 <code>4</code><br>   - 目標資料標頭起始列請填入 <code>3</code><br>   - 來源資料標頭起始列請填入 <code>1</code><br><br>接下來繼續下一步，將開始設定 \"欄位驗證條件\" 與 \"驗證結果輸出\"。",
        mfgTask2Step3Title: "任務二：欄位驗證設定 (3/4)",
        mfgTask2Step3Instruction: "在欄位與驗證條件功能區塊裡，首先要設定的是 \"目標工作表欄位\" 與 \"來源工作表欄位\" 的對應關係。<br>建議您可以優先使用 <code>自動對應驗證欄位</code> 功能，此功能將會 <b>自動匹配與推薦</b> 合適的標頭給您設定。<br>在此項任務裡，欄位驗證要設定的條件如下：<br>目標工作表欄位 - 來源工作表欄位<br><code>B</code> - <code>B</code><br><code>C</code> - <code>C</code><br><code>D</code> - <code>D</code><br><code>E</code> - <code>E</code><br><code>G</code> - <code>A</code>",
        ...
        mfgTask3Step3Instruction: "請套用以下設定，來關聯追蹤號碼並取得狀態：<br>1. **來源資料選擇**: 點擊「選取」按鈕，這會彈出一個檔案選擇視窗，請在視窗中選擇您當前的 Google 試算表檔案。<br>2. **來源資料分頁名稱**: <code>{SOURCE_SHEET_NAME}</code><br>3. **目標資料起始列數**: <code>4</code><br>4. **來源資料比對範圍**: <code>A2:B22</code> (涵蓋所有資料)<br>5. **目標查找欄位**: <code>H</code> (Shipment Tracking Number)<br>6. **來源比對欄位**: <code>A</code> (Shipment Tracking Number)<br>7. **來源返回欄位**: <code>B</code> (Shipment Status)<br>8. **目標寫入欄位**: <code>L</code> (Shipment Status)",
