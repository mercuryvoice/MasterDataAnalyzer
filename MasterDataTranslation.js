...
MasterData.TRANSLATIONS = {
    en: {
        ...
        sourceDataRangeLabel: 'Data Range (excluding header)',
    ...
    'zh_TW': {
        ...
        privacyDataCollectionL1: '<code>不收集個人資訊</code>：我們不會要求、收集或儲存任何您的個人身份資訊，例如姓名、電子郵件地址或聯絡資訊。',
        privacyDataCollectionL2: '<code>資料處理</code>：所有資料的讀取、處理與寫入操作，都直接在您授權的 Google Sheets 檔案中進行。資料不會被傳輸到我們的伺服器或任何外部服務。',
        privacyDataCollectionL3: "<code>設定儲存</code>：您為本外掛所做的設定（例如資料來源、欄位對應等）會使用 Google Apps Script 內建的 `PropertiesService` 儲存在與您的 Google 試算表綁定的屬性中。這些設定僅供本外掛在您的帳戶中運作時讀取，我們無法從外部存取。",
        ...
        privacyScopeGmailTitle: '`https://www.googleapis.com/auth/gmail.send`',
        privacyScopeGmailPermission: '權限：允許本外掛代表您傳送電子郵件。<code>（注意：本外掛無法讀取您的任何郵件）</code>',
        ...
        sourceDataRangeLabel: '資料範圍 (不包含標頭)',
        ...
        businessWelcomeInstruction: "歡迎來到 <code>業務統計範例</code> 的互動教學。\n\n我們的目標是將一份不完整的銷售紀錄，透過 MasterDataAnalyzer 的功能，變成一份完整的分析報表。\n\n點擊 <code>下一步</code> 開始我們的第一個任務！",
        ...
        businessTask1Step3Instruction: "在設定視窗中，請進行以下設定：\n1. <code>來源資料選擇</code>: 點擊「選取」按鈕，這會彈出一個檔案選擇視窗，請在視窗中選擇您當前的 Google 試算表檔案。\n2. <code>來源資料分頁名稱</code>: 選擇 <code>{SOURCE_SHEET_NAME}</code>\n3. <code>目標資料表分頁名稱</code>: 應會自動帶入 <code>{TARGET_SHEET_NAME}</code>",
        ...
        businessTask2Step1Instruction: "太棒了！現在儀表板有了原始數據，但還缺少客戶的詳細資訊。\n\n接下來，我們將使用<code>資料比對</code>功能，從 <code>[來源] 客戶清單</code> 中查找並填入客戶資料。\n\n打開 <code>資料匯入工具 > ⚙️ 資料比對設定</code>，並開始進行任務二的設定：",
        businessTask2Step2Title: "任務二：豐富客戶資料 (2/3)",
        businessTask2Step2Instruction: "請進行以下設定：\n1. <code>來源資料選擇</code>: 點擊「選取」按鈕，這會彈出一個檔案選擇視窗，請在視窗中選擇您當前的 Google 試算表檔案。接著選擇 <code>來源資料分頁名稱</code> 為 <code>{SOURCE_SHEET_NAME}</code>，而 <code>目標資料表分頁名稱</code> 為 <code>{TARGET_SHEET_NAME}</code>。\n2. <code>目標資料起始列數</code> 設定為 <code>2</code>，<code>來源資料比對範圍</code> 設定為 <code>A2:D6</code>。\n3. <code>資料比對欄位對應</code> 請依照下方順序逐一設定：\n  - <code>目標查找欄位</code>: <code>B</code> (客戶ID)\n  - <code>來源比對欄位</code>: <code>A</code> (客戶ID)\n  - <code>來源返回欄位</code>: <code>B</code> (客戶全名)\n  - <code>目標寫入欄位</code>: <code>C</code> (客戶全名)",
        ...
        businessTask3Step2Instruction: "這次，請用 <code>產品名稱</code> 作為目標對比基準，並選擇 <code>[來源] 產品目標</code> 為來源分頁。<br><code>來源資料比對範圍</code> 請設定為 <code>A2:B5</code>，<code>資料比對欄位對應</code> 請依照下方順序逐一設定：<br>  - <code>目標查找欄位</code>: <code>F</code> (產品名稱)<br>  - <code>來源比對欄位</code>: <code>A</code> (產品名稱)<br>  - <code>來源返回欄位</code>: <code>B</code> (目標月銷售額)<br>  - <code>目標寫入欄位</code>: <code>J</code> (目標月銷售額)",
        businessFinalStepTitle: "恭喜您，分析完成！",
        businessFinalStepInstruction: "接著，請再次執行<code>資料比對</code>。\n\n所有資料都已到位！您現在擁有了一份乾淨、完整的數據。\n\n接下來，您可以手動或點擊下方按鈕，在對應的欄位中填入公式來完成最後的計算：\n- <code>總銷售額</code>: <code>=H2*I2</code> (數量 * 單價)\n- <code>業績達成率</code>: <code>=G2/J2</code> (總銷售額 / 目標月銷售額)",
        ...
        businessTask4Step2Instruction: "在設定視窗中，請進行以下設定：\n1. <code>來源資料選擇</code>: 點擊「選取」按鈕，這會彈出一個檔案選擇視窗，請在視窗中選擇您當前的 Google 試算表檔案。\n2. <code>來源資料分頁名稱</code>: 選擇 <code>{SHEET_NAME}</code>\n3. <code>來源資料匯入範圍</code>: 請輸入需要分析的資料範圍如：<code>A1:K9</code>",
        ...
        businessTask4Step4Instruction: "太好了，這時您應可看到多維度與多指標的生成圖表報告了。請點擊 <code>確認匯出</code> 以在新的 Google Sheet 分頁中生成報表。",
        ...
        mfgWelcomeInstruction: "歡迎來到「生產製造範例」的互動教學。\n\n此教學將引導您使用 MasterDataAnalyzer 的 <code>資料匯入</code> 與 <code>資料驗證</code> 功能，將複雜的物料清單轉換為標準化格式，並與主資料進行比對。",
        ...
        mfgTask1Step3Instruction: "在設定視窗中，請進行以下設定：\n1. <code>選擇來源資料表</code>: 點擊 <code>選取</code> 按鈕，這會彈出一個檔案選擇視窗，請在視窗中選擇您當前的 Google 試算表檔案。\n2. <code>來源資料分頁名稱</code>: 選擇 <code>{SOURCE_SHEET_NAME}</code>\n3. <code>目標資料表分頁名稱</code>: 應會自動帶入 <code>{TARGET_SHEET_NAME}</code>",
        mfgTask1Step4Title: "任務一：資料匯入設定 (4/6)",
        mfgTask1Step4Instruction: "接下來是關鍵步驟，啟用「資料陣列比對」模式：\n1. <code>資料匯入的標頭之起始列數</code>: <code>3</code>\n2. <code>資料匯入的起始列數</code>: <code>4</code>\n3. <code>資料範圍 (不包含標頭)</code>: <code>A9:E14</code>\n4. <code>資料範圍 (包含標頭)</code>: <code>F2:I2</code>\n5. <code>其他區塊的標頭內的數據範圍</code>: <code>F9:I14</code>",
        mfgTask1Step5Title: "任務一：資料篩選設定 (5/6)",
        mfgTask1Step5Instruction: "在篩選與驗證功能區塊裡，您可以自行挑選想匯入的標頭與關鍵字篩選。\n1. 在 <code>來源標頭匯入範圍設定</code> 輸入 <code>A8:E8</code>\n2. 在 <code>關鍵字篩選條件 (AND)</code> 選擇 <code>Owner</code>\n3. 點擊關鍵字右側 <code>選取</code> 方塊並勾選 <code>全選</code>",
        ...
        mfgTask2Step2Instruction: "在 <code>資料驗證</code> 設定視窗中，請進行以下設定：<br>1. <code>選擇來源資料表</code>: 點擊 <code>選取</code> 按鈕，這會彈出一個檔案選擇視窗，請在視窗中選擇您當前的 Google 試算表檔案。<br>2. <code>來源資料分頁名稱</code>: 選擇 <code>{SOURCE_SHEET_NAME}</code><br>3. 請依照下列設定順序填妥資料範圍設定：<br>   - <code>資料驗證後匯入的起始列數</code> 請填入 <code>4</code><br>   - <code>目標資料標頭起始列</code> 請填入 <code>3</code><br>   - <code>來源資料標頭起始列</code> 請填入 <code>1</code><br><br>接下來繼續下一步，將開始設定 <code>欄位驗證條件</code> 與 <code>驗證結果輸出</code>。",
        mfgTask2Step3Title: "任務二：欄位驗證設定 (3/4)",
        mfgTask2Step3Instruction: "在欄位與驗證條件功能區塊裡，首先要設定的是 <code>目標欄位</code> 與 <code>來源欄位</code> 的對應關係。<br>建議您可以優先使用 <code>自動對應驗證欄位</code> 功能，此功能將會 <b>自動匹配與推薦</b> 合適的標頭給您設定。<br>在此項任務裡，欄位驗證要設定的條件如下：<br><code>目標欄位</code> - <code>來源欄位</code><br><code>B</code> - <code>B</code><br><code>C</code> - <code>C</code><br><code>D</code> - <code>D</code><br><code>E</code> - <code>E</code><br><code>G</code> - <code>A</code>",
        mfgTask2Step4Title: "任務二：驗證結果輸出設定 (4/4)",
        mfgTask2Step4Instruction: "接著設定當驗證成功時，要從來源回填的資料欄位。<br>同樣地，您也可以使用 <code>自動對應輸出欄位</code> 功能來加速設定。<br>驗證結果輸出要設定的欄位條件如下：<br>目標欄位 - 來源欄位<br><code>H</code> - <code>F</code><br><code>I</code> - <code>G</code><br><code>J</code> - <code>H</code><br><br>最後，我們將 <code>不吻合資訊輸出</code> 的欄位設為 <code>K</code>，以便腳本將錯誤訊息寫入此處。",
        ...
        mfgTask3Step3Instruction: "請套用以下設定，來關聯追蹤號碼並取得狀態：<br>- <code>來源資料選擇</code>: 點擊 <code>選取</code> 按鈕，並從檔案選擇器中選擇目前的試算表檔案。<br>- <code>來源資料分頁名稱</code>: <code>{SOURCE_SHEET_NAME}</code><br>- <code>目標資料起始列數</code>: <code>4</code><br>- <code>來源資料比對範圍</code>: <code>A2:B22</code> (涵蓋所有資料)<br>- <code>目標查找欄位</code>: <code>H</code> (Shipment Tracking Number)<br>- <code>來源比對欄位</code>: <code>A</code> (Shipment Tracking Number)<br>- <code>來源返回欄位</code>: <code>B</code> (Shipment Status)<br>- <code>目標寫入欄位</code>: <code>L</code> (Shipment Status)",
        ...
        noDataTitle: "無資料可處理",
        noDataBody: "目標工作表中沒有找到任何資料列可供驗證。\n\nPlease ensure there is data below the configured <code>Data Start Row</code>.",
